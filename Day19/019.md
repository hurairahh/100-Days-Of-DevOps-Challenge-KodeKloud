...
- `Apache` should serve on port `6200`.
- There are two website's backups `/home/thor/media` and `/home/thor/demo` on `jump_host`. Set them up on `Apache` in a way that official should work on the link `http://localhost:6200/media/` and games should work on link `http://localhost:6200/demo/` on the mentioned app server.
- Once configured you should be able to access the website using curl command on the respective app server, i.e `curl http://localhost:6200/media/` and `curl http://localhost:6200/demo/`
...
2. Change Apache port:
    ...
    sudo sed -i 's/80/6200/g' /etc/httpd/conf/httpd.conf
    ```
...
4. Copy backup from Jump Host to App Server
    ...
    ```sh
    scp -r /home/thor/media banner@stapp03:/home/banner
    scp -r /home/thor/demo banner@stapp03:/home/banner/
    ```

5. Place websites

    ```sh
    sudo cp -r /home/banner/media /var/www/html/
    sudo cp -r /home/banner/demo /var/www/html
    ```
