...
- Create a database user `kodekloud_pop` and set its password to `your-password`.
- Create a database `kodekloud_db8` and grant full permissions to user `kodekloud_pop` on this database.
...
2. Switch to postgres user and run following commands:
    ...
    psql -c "CREATE ROLE kodekloud_pop LOGIN PASSWORD 'your-password';"
    psql -c "GRANT ALL PRIVILEGES ON DATABASE kodekloud_db8 TO kodekloud_pop;"
    psql -c "ALTER DATABASE kodekloud_db8 OWNER TO kodekloud_pop;"
    ```
