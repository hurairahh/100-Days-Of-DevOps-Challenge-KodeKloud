...
> Create a user named `anita` on `App Server 1` in Stratos Datacenter. Set the expiry date to `2027-04-15`, ensuring the user is created in lowercase as per standard protocol.

## Steps

1. Follow the Day01 to connect server and run the following command:

    ```sh
    sudo useradd -m -e 2027-04-15 anita
    ```
